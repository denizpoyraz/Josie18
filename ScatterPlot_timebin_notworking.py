...
for i in range(len(dfmd)):
    ...
    ENSCI.append(int(dfmd.iloc[i]['ENSCI']))
...
slope_sim_time = np.zeros((len(Sim), nt))
intercept_sim_time = np.zeros((len(Sim), nt))
slope_sim_time_err = np.zeros((len(Sim), nt))
...
for s in range(3):
    ...
    # for each simulation now i want to make a fitting for 500 seconds of time bins
    for tt in range(nt):
        filtt = (df_sim[s].Tsim > minTbin[tt]) & (df_sim[s].Tsim <= maxTbin[tt])
        filterlist2.append(filtt)
        df_tbin[tt] = df_sim[s][filtt]
        
        #opm_sim.append(np.asarray(df_tbin[tt].PO3_OPM))
        #po3_sim.append(np.asarray(df_tbin[tt].PO3))
        
        opm_sim[tt] = np.asarray(df_tbin[tt].PO3_OPM)
        po3_sim[tt] = np.asarray(df_tbin[tt].PO3)
        print(s, tt, opm_sim[tt].mean())
        ...
        #print('tt', tt, slope[tt])
        slope_sim_time[s][tt] = slope[tt]
        intercept_sim_time[s][tt] = intercept[tt]
        slope_sim_time_err[s][tt] = std_err[tt]
        #print('s', s, ' tt ', tt)

#for xt in range(nt):
#    print(' 0 ' , xt, slope_sim_time[0][xt])
#    print(' 1 ' , xt, slope_sim_time[1][xt])
